...
def index():
    if session:
        ...
        passe = session['password']
        print("admin")
        print(username)
        print(passe)
        print(session)
        if username=="admin" and passe=="admin":
...
def login():
    if request.method == 'POST':
        requete_liste_promo = "SELECT username,passe FROM authentification"
...
def lister_apprenant():
    if session:
        ...
        requete_liste_promo = "SELECT id_promo,libelle FROM promotion WHERE date_fin>=DATE( NOW() )"
        curseur.execute(requete_liste_promo)
...
def lister_appar_promo():
    if session:
        ...
        if id_promo == "tous":
            requete_liste_app = """SELECT matricule,prenom,nom,sexe,lieu_naissance,adresse,
                                    telephone,email,statut,promotion.libelle FROM apprenant,promotion 
                                    WHERE promotion.id_promo = apprenant.id_promo and date_fin>=DATE( NOW() )
...
def lister_appro_suspendre():
    if session: 
        ...
        if id_promo == "tous":
            requete_liste_app = """SELECT matricule,prenom,nom,sexe,date_naissance,lieu_naissance,adresse,
                                telephone,email,promotion.libelle,statut FROM apprenant,promotion 
                                WHERE promotion.id_promo=apprenant.id_promo and statut = 'suspendre' """
            curseur.execute(requete_liste_app,(id_promo,))
            result=curseur.fetchall()
